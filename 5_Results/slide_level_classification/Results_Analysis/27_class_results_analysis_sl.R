...
results_dirs = dir(dir_loc, pattern="thumb")
...
for (i in 1:length(results_dirs)) {
  ...
  files_in_dir = list.files(paste0(dir_loc, this_dir), pattern="csv")
  ...
  eval(parse(text=paste0(df_name, "$true_class = str_extract(", df_name, "$filenames, pattern='DX|(B|M|T)S')")))
  eval(parse(text=paste0(df_name, "$true_class[", df_name, "$true_class!='DX'] = 'PM'")))
  eval(parse(text=paste0(df_name, "$predicted_idx = unlist(apply(", df_name, "[,c(3:4)], 1, function(x) {which.max(x)}))")))
  ...
  eval(parse(text=paste0(df_name, "$predicted_prob = sapply(1:nrow(", df_name, "), function(x){", df_name, "[x, 2+", df_name, "$predicted_idx[x]]})")))
  ...
  eval(parse(text=paste0("data = ", df_name, "[, c(5, 7)]")))
